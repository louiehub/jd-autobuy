...
class JDWrapper(object):
    ...
    def __init__(self, usr_name=None, usr_pwd=None):
        ...
        self.sess.headers = self.get_headers()
    ...
    def get_pay_headers():
        ...
        #     "referrer": "https://trade.jd.com/shopping/order/getOrderInfo.action",
        #     "referrerPolicy": "strict-origin-when-cross-origin",
        #     "method": "POST",
        #     "mode": "cors"
        # }
    ...
    def login_by_QR(self):
        ...
        try:
            ...
            # url:
            # https://safe.jd.com/dangerousVerify/index.action?username=...
    ...
    def good_stock(self, stock_id, good_count=1, area_id=None):
        ...
        try:
            ...
            # 33 : on sale, 34 : out of stock, 36: presell, 40: allocate
    ...
    def buy(self, options):
        ...
        while good_data['stock'] != 33 and good_data['stock'] != 40 and options.flush:
        ...
        # 33 现货, 40 可调货
        if good_data['stock'] != 33 and good_data['stock'] != 40:
            print(u'stock {0}, link {1}'.format(good_data['stock'], link))
            return False

        # failed! outdated!
        # link = good_data['link']
        link = "https://cart.jd.com/gate.action?pid=%s&pcount=1&ptype=1" % options.good

        try:
            # change buy count
            if options.count != 1:
                link = link.replace(
                    'pcount=1', 'pcount={0}'.format(options.count))

            # add to cart
            resp = self.sess.get(link)
            soup = bs4.BeautifulSoup(resp.text, "html.parser")
            with open('rsp.txt', 'w') as f:
                f.write(resp.text)
    ...
    def cart_detail(self):
        ...
        try:
            ...
            for item in soup.select('div.item-form'):
                check = tags_val(item.select(
                    'div.cart-checkbox input'), key='checked')
                check = ' + ' if check else ' - '
                count = tags_val(item.select(
                    'div.quantity-form input'), key='value')
                ...
                #: ￥字符解析出错, 输出忽略￥
                print(cart_format.format(
                    check, count, price[1:], sums[1:], gname))
    ...
    def order_info(self, submit=False):
        ...
        try:
            ...
            # red packet request
            payload = {"use": False, "flowType": 0, "preSalePaymentTypeInOptional": 0,
                       "overseaMerge": 0, "presaleStockSign": 1}
            red_packet_url = 'https://trade.jd.com/virtual/v1/red-packet'
            rp = self.sess.put(red_packet_url, params=payload, headers=self.get_pay_headers())

            # # check payment
            # payload = {'presaleStockSign': '1'}
            # check_payment_url = 'https://trade.jd.com/shopping/dynamic/order/checkPaymentPasswordComb.action'
            # rp = self.sess.post(red_packet_url, params=payload)
            ...
            order_url = 'https://trade.jd.com/shopping/order/submitOrder.action?presaleStockSign=1'
            rp = self.sess.post(order_url, params=payload, headers=self.get_pay_headers())
            ...
            if rp.status_code == 200:
                ...
                if js['success'] == True:
                    ...
                    print(u'下单失败！<{0}: {1}>'.format(
                        js['resultCode'], js['message']))
    ...
    def collect_coupon(self, activity_id, coupon_args, submit=False):
        # get order info detail, and submit order
        print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++')
        print(u'{0} > 开始抢券'.format(time.ctime()))

        try:
            coupon_url = 'https://api.m.jd.com/client.action?functionId=newBabelAwardCollection'
            ...
            payload = {
                ...
                "eid": self.eid,
                "fp": self.fp,
                "shshshfp": self.shshshfp,
                "shshshfpa": self.shshshfpa,
                "shshshfpb": self.shshshfpb,
                "childActivityUrl": "https://pro.m.jd.com/mall/active/%s/index.html?suggest=1.prom.0.base&wq=图书" % activity_id,
            ...
            rp = self.sess.post(coupon_url, params=payload,
                                cookies=self.cookies)

            # {subCodeMsg: "本时段优惠券已抢完，请19:59再来吧！", subCode: "D2", code: "0", msg: null}
            # {subCodeMsg: "领取成功！感谢您的参与，祝您购物愉快~", subCode: "A1", code: "0", couponId: "129463089117", msg: null}
            if rp.status_code == 200:
                js = json.loads(rp.text)
                if js['subCode'] == 'A1':
                    print(u'抢券成功！卡券编号：{0}'.format(js['couponId']))
                    return True
                else:
                    print(u'抢券失败！<{0}: {1}>'.format(
                        js['subCode'], js['subCodeMsg']))
                    if js['resultCode'] == '60017':
                        # 60017: 您多次提交过快，请稍后再试
                        time.sleep(0.3)
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--area',
                        help='Area string, like: 2_2825_51931_0 for Shanghai', default='2_2825_51931_0')
    parser.add_argument('-g', '--good',
                        help='Jing Dong good ID', default='100002581159')
    ...
    hw_watch = '2567304'
    iphone_7 = '3133851'
    ps4_pro_limited = '8894283'
    jelly = '859582'
    jelly_pack = '100002581159'
    earphone = '100006556983'
